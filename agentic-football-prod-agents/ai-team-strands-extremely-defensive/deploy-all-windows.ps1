...
$ErrorActionPreference = "Continue"
Write-Host "  Running: agentcore create --name dfs_gk_agent --project-name $projectName --output-dir $buildDirName"
& $agentcorePath create --name "dfs_gk_agent" --project-name $projectName --build CodeZip --framework Strands --model-provider Bedrock --memory none --skip-git --skip-python-setup --output-dir $buildDirName 2>&1 | ForEach-Object { Write-Host "    $_" }
...
if ($deployExitCode -eq 0) {
    ...
    foreach ($agent in $allAgents) {
        $agentNameClean = "dfs_" + ($agent -replace 'ai-', '') + "_agent"
        $match = $allRuntimes | Where-Object { $_.name -eq "aiteamdefensive_$agentNameClean" }
        if ($match) {
