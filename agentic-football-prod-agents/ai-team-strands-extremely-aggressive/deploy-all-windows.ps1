<#
.SYNOPSIS
    Deploy all 5 AI Team (Extremely Aggressive) agents using the AgentCore CLI (@aws/agentcore)
.DESCRIPTION
...
Write-Host ""
Write-Host "=========================================="
Write-Host "  AI Team (Extremely Aggressive) - Deploy All Agents (New CLI)"
Write-Host "=========================================="
Write-Host ""
...
foreach ($agent in $allAgents) {
    $agentNameClean = "agg_" + ($agent -replace 'ai-', '') + "_agent"
...
if ($deployExitCode -eq 0) {
    ...
    foreach ($agent in $allAgents) {
        $agentNameClean = "agg_" + ($agent -replace 'ai-', '') + "_agent"
        $match = $allRuntimes | Where-Object { $_.name -eq "aiteamaggressive_$agentNameClean" }
        if ($match) {
